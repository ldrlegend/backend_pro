...
class Product(Base):
    ...
    product_attribute_value_index = relationship("ProductAttributeValueIndex", back_populates="product", cascade="all, delete-orphan")
...
# @property
    ...
    @property
    def attributes(self):
        """Get all dynamic attributes for this product"""
        return {
            pavi.attribute.attribute_code: {
                'attribute': pavi.attribute,
                'option': pavi.attribute_option,
                'value': pavi.attribute_value
            } 
            for pavi in self.product_attribute_value_index
        }
